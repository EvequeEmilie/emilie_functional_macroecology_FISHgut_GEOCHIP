...
fit_lmm <- function(input_data, model_list, nm_var_Y = NULL) {
  ...
  if (! is.null(nm_var_Y)) {
    ...
    res$mods <- map(model_list, 
                    ~ lmer(formula(.x), data = input_data)
    )
  }
...
# 
# 
# extract_best_lm <- function(X, force_linear = FALSE) {
#   
#   # keep only the models different from model 1
#   mask <- X$mod_comp[, "Pr(>F)"] < 0.05
#   mask[is.na(mask)] <- TRUE
#   mask <- rep(TRUE, 3)
#   
#   if (sum(mask, na.rm = T) != 0) {
...
#   # result of ANOVA 
#   tmp <- t(X$anova[[bm]])
#   tmp <- tmp[, ! colnames(tmp) %in% "Residuals"]
#   f <- tmp["F value", ] %>% setNames(paste0("Fval_", names(coeff)[-1]))
#   p <- tmp["Pr(>F)",] %>% setNames(paste0("Pval_", names(coeff)[-1]))
#   f_and_p <- rep(NA, 4) %>% setNames(c(names(f), names(p)))
#   f_and_p[names(f)] <- f
